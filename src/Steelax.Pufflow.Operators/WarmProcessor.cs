...
/// <list type="bullet">
/// <item><description>a per-key delayed buffer (the <c>FlowGate</c> dictionary): values of warmable
/// keys are held back in their <see cref="WarmAccumulator{TValue,TGroup}"/> until the warm result is
/// ready, so they are not emitted out of order;</description></item>
/// <item><description>the <see cref="Warmer{TKey,TWarm}"/>: key segmentation, bounded concurrent warm
/// jobs, and head-of-line (watermark-ordered) emission — it is the watermark barrier.</description></item>
/// </list>
/// <para/>
    ...
    where TKey : notnull
{
    ...
    public WarmProcessor(
        ...
        IWarmPolicy<TKey, TWarm> policy,
        IWarmAccumulatorFactory<TKey, TValue, TGroup> accumulatorFactory,
        long queueWeightLimit) : this(warmer, keySelector, policy, accumulatorFactory, queueWeightLimit, Timeout.InfiniteTimeSpan)
    {
    }
    ...
    /// </summary>
    [PublicAPI]
    public IAsyncConsumator<Unio<TValue, TGroup, Watermark>> GetAsyncConsumator(IAsyncEnumerator<Watermarked<TValue>> source, FlowContext context)
    {
        var consumator = new AsyncEnumeratorToAsyncConsumator<Watermarked<TValue>, IAsyncEnumerator<Watermarked<TValue>>>(source, _fanIn);
        ...
        where TConsumator : IAsyncConsumator<Watermarked<TValue>>, ICursorable<Watermarked<TValue>>
    {
        ...
        try
        {
            while (!context.Token.IsCancellationRequested)
            {
                ...
                if (sourceCompleted && _warmer.IsEmpty && _delayedQueue.Count == 0)
                {
                    break;
                }
