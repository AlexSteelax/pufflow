...
public readonly struct Watermarked<T> : IEquatable<Watermarked<T>>
{
    ...
    [PublicAPI]
    public static implicit operator Watermarked<T>(T value) => new(value, Watermark.Nothing());
    ...
    [PublicAPI]
    public static implicit operator T(Watermarked<T> source) => source._value;
    ...
    public bool Equals(Watermarked<T> other) =>
        _watermark == other._watermark && EqualityComparer<T>.Default.Equals(_value, other._value);

    /// <summary>
    /// Indicates whether the current watermarked value equals another object.
    /// </summary>
    /// <param name="obj">The object to compare with.</param>
    /// <returns><see langword="true"/> when <paramref name="obj"/> is a <see cref="Watermarked{T}"/> with an equal value and watermark.</returns>
    [PublicAPI]
    public override bool Equals(object? obj) => obj is Watermarked<T> other && Equals(other);
    ...
    [PublicAPI]
    public override int GetHashCode() => HashCode.Combine(_watermark, _value);
    ...
    public static bool operator ==(Watermarked<T> left, Watermarked<T> right) => left.Equals(right);
    /// <summary>
