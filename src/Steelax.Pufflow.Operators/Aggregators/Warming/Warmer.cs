...
///     </para>
/// </remarks>
internal sealed class Warmer<TKey, TWarm> : IDisposable, IAsyncDisposable
{
    ...
    public bool CanAdd =>
        !_queue.IsFull ||
        (_queue.TryPeekLast(out var tail) && tail.CanAccept);
    ...
    {
        return _queue.TryPeekLast(out var tail) && tail.CanAccept;
    }
    ...
    private bool TryExtractHead<TWarmable>(TWarmable warmable, [MaybeNullWhen(false)] out TKey[] keys,
        out Watermark watermark)
        where TWarmable : IWarmPolicy<TKey, TWarm>
    {
        var headExist = _queue.TryPeekFirst(out var head);
