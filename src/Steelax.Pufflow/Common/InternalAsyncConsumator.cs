    ...
    where TAsyncConsumator : IAsyncConsumator<T>
{
    ...
    ///     An optional cancellation token (currently unused but required for signature
    ///     compatibility).
    /// </param>
    /// <returns>The underlying <typeparamref name="TAsyncConsumator" /> instance.</returns>
    public TAsyncConsumator GetAsyncConsumator(CancellationToken cancellationToken = default)
    {
        return consumator;
    }
}