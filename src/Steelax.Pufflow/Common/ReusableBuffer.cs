...
/// <summary>
/// Represents a reusable fixed-capacity buffer that implements both <see cref="IEnumerable{T}"/> and <see cref="IEnumerator{T}"/>.
...
/// <remarks>
/// <para>The same instance serves as both the enumerable and the enumerator, avoiding a separate allocation
/// when enumerated via <see langword="foreach"/>.</para>
/// <para>This type is intended for bridging between push-based data sources (e.g., async streams)
/// and pull-based sync consumers via batch processing.</para>
...
internal sealed class ReusableBuffer<T>(int capacity) : IEnumerable<T>, IEnumerator<T>
{
    ...
    /// </summary>
    /// <value>The number of elements added via <see cref="TryAdd"/> since the last <see cref="Reset"/> or <see cref="Dispose"/>.</value>
    [PublicAPI]
    ...
    /// <returns><see langword="true"/> if the item was successfully added; otherwise, <see langword="false"/> if the buffer is full.</returns>
    [PublicAPI]
    ...
    /// </summary>
    /// <returns>The same <see cref="ReusableBuffer{T}"/> instance, enabling reuse as both <see cref="IEnumerable{T}"/> and <see cref="IEnumerator{T}"/>.</returns>
    /// <remarks>
    ...
    [PublicAPI]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public ReusableBuffer<T> GetEnumerator() => this;
